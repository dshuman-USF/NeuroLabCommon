...
typedef struct {
  ...
  float char_height, char_width, line_r, line_g, line_b;
...
sb_world_to_phys (WinData *w, float *x, float *y, int *phys_x, int *phys_y)
{
  ...
  do_port (w);
  vdc_x = w->port_x1 + (double)(w->port_x2 - w->port_x1) / (w->window_x2 - w->window_x1) * (*x - w->window_x1);
  vdc_y = w->port_y1 + (double)(w->port_y2 - w->port_y1) / (w->window_y2 - w->window_y1) * (*y - w->window_y1);
...
void
sb_text2d (int *fildes, float *x_in, float *y_in, char *string_in, int *xform, int *more, int slen)
{
  text (*fildes, *x_in, *y_in, string_in, *xform, *more, 1, slen);
}

void
sb_append_text (int *fildes, char *string_in, int *xform, int *more, int slen)
{
...
sb_request_locator (long *fildes, int *ordinal, float *ftimeout, int *valid, float *x, float *y, float *z)
{
  ...
  while (1) {
	...
	*z = 0;
	...
	int len = 10, cnt;
	char buf[len];
	cnt = XLookupString (&e.xkey, buf, len, &ks, 0);
	*valid = 2;
	*ordinal = buf[0];
	return;
      }
...
void
sb_set_locator (int *fildes, int *ordinal, int *x, int *y, int *z)
{
}
...
void
sb_echo_type (int *fildes, int *echo_number, int *echo_value, float *x, float *y, float *z)
{
}
...
long
sb_gopen (int *width_p, int *height_p, int *x_p, int *y_p, char* title_in)
{
  int x = *x_p, y = *y_p;
...
void
append_text (int *fildes, char *string_in, int *xform, int *more, int slen)
{
  if (p) {
...
void
text2d (int *fildes, float *x_in, float *y_in, char *string_in, int *xform, int *more, int slen)
{
  if (p) {
    char *s = malloc (slen + 1);
    memcpy (s, string_in, slen);
...
gopen (int *width_p, int *height_p, int *x_p, int *y_p, char* title_in, int len)
{
  ...
  else   return sb_gopen (width_p, height_p, x_p, y_p, title_in);
}
...
void
echo_type (int *fildes, int *echo_number, int *echo_value, float *x, float *y, float *z)
{
  if (p) ;
...
void
set_locator (int *fildes, int *ordinal, int *x, int *y, int *z)
{
  if (p) ;
